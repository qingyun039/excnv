...
from cnvlib import core, descriptives
...
def do_reference_flat(binsize, fa_fname=None,
                      bw_fname=None,
    ...
    if fa_fname:
        ...
        genome = 'hg19'
    ref_probes = genome2probes(binsize, genome)
    ...
    if fa_fname:
        gc, rmask, ns = get_fasta_stats2(ref_probes, fa_fname)
        ref_probes['gc'] = gc
    ...
    if bw_fname:
        ab = get_region_alignability(ref_probes, bw_fname)
        ref_probes['ab'] = ab
    else:
        logging.info("No Alignability BigWig file provided; "
                     "skipping Alignablility calculations")
...
def genome2probes(binsize, genome):
    """Create a neutral-coverage CopyNumArray from a genome"""
    if genome[-3:] == '.fa' or genome[-6:] == '.fasta':
        fafile = fasta_extract_lens(genome)
        pybedtools.genome_registry.fafile = fafile
        genome = 'fafile'
    elif genome not in ('hg18', 'hg19', 'hg38'):
        raise ValueError("Expected FASTA file or (hg18, hg19, hg38), but get %s" % genome)
        
    bed_fname = make_windows(binsize, genome)
    ref_probes = bed2probes(bed_fname)
    return ref_probes

def make_windows(binsize=10000, genome='hg19', chroms=params.CHROMS):
    """Split genome into BIN"""
    logging.info("Spliting genome %s into %d bins" % (genome, binsize))
    gb = (pybedtools.BedTool()
            .window_maker(genome=genome, w=binsize, s=binsize)
            .filter(lambda x: x.chrom in chroms)
            .saveas())
    return gb.fn

def do_reference(bamfiles, binsize, fa_fname=None,
                 male_reference=False, female_samples=None,
                 do_gc=True, do_edge=False, do_rmask=True, do_cluster=False,
                 min_cluster_size=4, bw_fname=None,
                 by_count=False, min_mapq=0, processes=1):
    ...
    if not fa_fname:
        ...
        genome = fa_fname
    ref_probes = genome2probes(binsize, genome)
    bedfile = fix.probes2bed(ref_probes)
    samples = []
    for bamfile in bamfiles:
        cnarr = coverage.do_coverage(bedfile, bamfile, by_count=by_count, min_mapq=min_mapq, processes=processes, fasta=fa_fname)
        samples.append(cnarr)
    ...
    if female_samples is None:
        ...
        sexes = infer_sexes(samples, False)
    else:
        sexes = collections.defaultdict(lambda: female_samples)

    # TODO - refactor/inline this func here, once it works
    ref_probes = combine_probes(samples, fa_fname,
                                male_reference, sexes, do_gc, do_edge, do_rmask,
                                do_cluster, min_cluster_size, bw_fname)
    reson = warn_bad_bins(ref_probes)
    ref_probes['filter'] = reson
    return ref_probes


def infer_sexes(cnarrs, is_haploid_x):
...
def combine_probes(cnarrs, fa_fname,
    ...
    ----------
    samples : list
        List of samples cnarr, corresponding to targetcoverage.cnn and
    ...
    ref_df, all_logr, all_depths = load_sample_block(
        cnarrs, fa_fname, is_haploid_x, sexes,
        True, fix_gc, fix_edge, fix_rmask, bw_fname)
    ...
    if do_cluster:
        ...
        sample_ids = [cnarr.sample_id for cnarr in cnarrs]
...
def load_sample_block(cnarrs, fa_fname,
                      is_haploid_x, sexes, skip_low,
                      fix_gc, fix_edge, fix_rmask, bw_fname):
    ...
    filenames = sorted(cnarrs, key=lambda x: x.sample_id)
    ...
    if fa_fname and (fix_rmask or fix_gc):
        gc, rmask, ns = get_fasta_stats2(cnarr1, fa_fname)
        if fix_gc:
            ref_columns['gc'] = gc
        if fix_rmask:
        ...
        gc = cnarr1['gc']
        ref_columns['gc'] = gc
    if bw_fname and fix_rmask:
        ab = get_region_alignability(cnarr1, bw_fname)
        ref_columns['ab'] = ab
    ...
    for cnarrx in cnarrs[1:]:
        logging.info("Loading %s", cnarrx.sample_id)
        ...
        if not np.array_equal(
            ...
            raise RuntimeError("%s bins do not match those in %s"
                               % (cnarrx.sample_id, cnarr1.sample_id))
...
def bias_correct_logr(cnarr, ref_columns, ref_edge_bias,
    ...
    if (cnarr['log2'] > params.NULL_LOG2_COVERAGE - params.MIN_REF_COVERAGE
        ...
        if 'ab' in ref_columns and fix_rmask:
            logging.info("Correcting for Alignability bias...")
            cnarr = fix.center_by_window(cnarr, .1, ref_columns['ab'])
        elif 'rmask' in ref_columns and fix_rmask:
...
def summarize_info(all_logr, all_depths):
    ...
    depth_centers = np.apply_along_axis(descriptives.biweight_location, 0,
                                        all_depths)
    zero_frac = np.apply_along_axis(lambda x: (x == 0).sum() / len(x), 0,
                                    all_depths)
...
def warn_bad_bins(cnarr, max_name_width=50):
    ...
    """
    mask, reson = fix.mask_bad_bins2(cnarr)
    bad_bins = cnarr[mask]
...
def get_fasta_stats2(cnarr, fa_fname):
    """Calculate GC and RepeatMasker content of each bin in the FASTA genome."""
    logging.info("Calculating GC, RepeatMasker, N content in %s ...", fa_fname)
    gc_rm_vals = [calculate_gc_lo_ns(subseq)
                  for subseq in fasta_extract_regions(fa_fname, cnarr)]
    gc_vals, rm_vals, ns_vals = zip(*gc_rm_vals)
    return np.asfarray(gc_vals), np.asfarray(rm_vals), np.asfarray(ns_vals)

def get_region_alignability(cnarr, bw_fname):
    """Calculate Alignability of each bin in FASTA genome."""
    logging.info("Using %s to calculate Alignability for each bin.", bw_fname)
    bw = pyBigWig.open(bw_fname)
    ab = cnarr.data.apply(lambda x: bw.stats(x[0], x[1], x[2])[0] or 0.0, axis=1)
    return np.asfarray(ab)
...
def calculate_gc_lo_ns(subseq):
    """Calculate the GC, lowercase (RepeatMasked), N content of a string."""
    cnt_at_lo = subseq.count('a') + subseq.count('t')
    cnt_at_up = subseq.count('A') + subseq.count('T')
    cnt_gc_lo = subseq.count('g') + subseq.count('c')
    cnt_gc_up = subseq.count('G') + subseq.count('C')
    cnt_ns = subseq.count('N') + subseq.count('n')
    tot = float(cnt_gc_up + cnt_gc_lo + cnt_at_up + cnt_at_lo)
    if not tot:
        frac_gc, frac_lo = 0.0,  0.0
    else:
        frac_gc = (cnt_gc_lo + cnt_gc_up) / tot
        frac_lo = (cnt_at_lo + cnt_gc_lo) / tot
    frac_ns = cnt_ns / len(subseq)
    return frac_gc, frac_lo, frac_ns
...
def fasta_extract_lens(fa_fname):
    """Extract chromsome len from indexed FASTA file"""
    tmp = collections.OrderedDict()
    with pyfaidx.Faidx(fa_fname) as f:
        for chrom in f.index:
            tmp[chrom] = (0, f.index[chrom][0])
    return tmp
