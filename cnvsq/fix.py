...
from cnvlib import descriptives, smoothing
from . import params, coverage 

def do_fix(bamfile, reference,
           do_gc=True, do_edge=False, do_rmask=True, do_cluster=False,
           by_count=False, min_mapq=0, processes=1, fa_fname=None):
    ...
    bedfile = probes2bed(reference)
    sample = coverage.do_coverage(bedfile, bamfile, by_count=by_count, min_mapq=min_mapq, processes=processes, fasta=fa_fname)
    logging.info("Processing sample: %s", sample.sample_id)
    cnarr, ref_matched = load_adjust_coverages(sample, reference,
                                               True, do_gc, do_edge, do_rmask)
    ...
    cnarr.data.loc[cnarr.data['log2'] < params.MIN_REF_COVERAGE, 'log2'] = params.MIN_REF_COVERAGE
...
def load_adjust_coverages(cnarr, ref_cnarr, skip_low,
    ...
    if (cnarr['log2'] > params.NULL_LOG2_COVERAGE - params.MIN_REF_COVERAGE
        ...
        if fix_edge:
            ...
            cnarr_index_reset = True
        if fix_rmask:
            if 'ab' in ref_matched:
                logging.info("Correcting for Alignability bias...")
                cnarr = center_by_window(cnarr, .1, ref_matched['ab'])
                cnarr_index_reset = True
            elif 'rmask' in ref_matched:
...
def mask_bad_bins(cnarr):
    ...
    if 'depth' in cnarr:
        mask |= cnarr['depth'] == 0
    if 'zero' in cnarr:
        mask |= ((cnarr.chromosome != cnarr._chr_x_label) & 
                 (cnarr.chromosome != cnarr._chr_y_label) & 
                 (cnarr['zero'] > params.MAX_ZERO_DEPTH))
    if 'rmask' in cnarr:
        mask |= cnarr['rmask'] > params.MAX_RMASK
    if 'ab' in cnarr:
        mask |= cnarr['ab'] < params.MIN_ALIGNABILITY
    if 'gc' in cnarr:
        mask |= (cnarr['gc'] > params.MAX_GC) | (cnarr['gc'] < params.MIN_GC)
    if 'ns' in cnarr:
        mask |= cnarr['ns'] > params.MAX_N_CONTENT
    return mask

def mask_bad_bins2(cnarr):
    """Flag the bins with excessively low or inconsistent coverage.
    ...
    np.array
        A boolean array where True indicates bins that failed the checks.
    pd.Series
        A Series to indicates filtered reson
    """
    reson = pd.Series(['PASS'] * len(cnarr))
    filter = ((cnarr['log2'] < params.MIN_REF_COVERAGE) |
              (cnarr['log2'] > -params.MIN_REF_COVERAGE))
    reson[filter] = 'COVERAGE'
    mask = filter

    filter = (cnarr['spread'] > params.MAX_REF_SPREAD)
    reson[filter] = 'SPREAD'
    mask |= filter

    if 'depth' in cnarr:
        filter = (cnarr['depth'] == 0)
        reson[filter] = 'DEPTH'
        mask |= filter
    if 'zero' in cnarr:
        filter = ((cnarr.chromosome != cnarr._chr_x_label) & 
                  (cnarr.chromosome != cnarr._chr_y_label) & 
                  (cnarr['zero'] > params.MAX_ZERO_DEPTH))
        reson[filter] = 'ZERO'
        mask |= filter
    if 'rmask' in cnarr:
        filter = (cnarr['rmask'] > params.MAX_RMASK)
        reson[filter] = 'RMASK'
        mask |= filter
    if 'ab' in cnarr:
        filter = (cnarr['ab'] < params.MIN_ALIGNABILITY)
        reson[filter] = 'ALIGNABILITY'
        mask |= filter
    if 'gc' in cnarr:
        filter = ((cnarr['gc'] > params.MAX_GC) | (cnarr['gc'] < params.MIN_GC))
        reson[filter] = 'GC'
        mask |= filter
    if 'ns' in cnarr:
        filter = (cnarr['ns'] > params.MAX_N_CONTENT)
        reson[filter] = 'N'
        mask |= filter
...
def probes2bed(refarr):
    """Convert cnarr to BedTool"""
    regions = refarr.data.loc[:, ('chromosome', 'start', 'end')]
    bed = pybedtools.BedTool(regions.to_string(header=False, index=False), from_string=True)
    bed.saveas()
    return bed.fn
